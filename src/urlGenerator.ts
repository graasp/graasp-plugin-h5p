...
export default class UrlGenerator implements IUrlGenerator {
  ...
  public contentUserData = (user: IUser): string => {
    if (this.csrfProtection?.queryParamGenerator && this.csrfProtection?.protectContentUserData) {
      const qs = this.csrfProtection.queryParamGenerator(user);
  ...
  public coreFile = (file: string): string =>
    `${this.baseUrl()}${this.config.coreUrl}/${file}?version=${this.config.h5pVersion}`;
  ...
  public editorLibraryFile = (file: string): string =>
    `${this.baseUrl()}${this.config.editorLibraryUrl}/${file}?version=${this.config.h5pVersion}`;

  public editorLibraryFiles = (): string => `${this.baseUrl()}${this.config.editorLibraryUrl}/`;

  public libraryFile = (library: any, file: string): string => {
  ...
  public setFinished = (user: IUser): string => {
    if (this.csrfProtection?.queryParamGenerator && this.csrfProtection?.protectSetFinished) {
      const qs = this.csrfProtection.queryParamGenerator(user);
  ...
  public temporaryFiles = (): string => this.baseUrl() + this.config.temporaryFilesUrl;
